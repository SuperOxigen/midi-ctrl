...
/* TODO: Make this not a crap "if" list. */
bool_t MidiControlNumberIsDefined(midi_control_number_t control_number) {
  ...
  if (control_number == 0x03 || control_number == 0x29) return false;
  ...
  if ((control_number >= 0x14 && control_number <= 0x1F) ||
      (control_number >= 0x34 && control_number <= 0x3F)) return false;
  if (control_number == 0x55) return false;
  if (control_number == 0x56) return false;
  if (control_number == 0x57) return false;
  if (control_number == 0x59) return false;
  if (control_number == 0x5A) return false;
  if (control_number >= 0x66 && control_number <= 0x77) return false;
  return true;
}
